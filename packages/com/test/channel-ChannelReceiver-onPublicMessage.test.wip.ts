...
it("doens't throw on invalid message received", (t) => {
	const channelReceiver = new StandaloneChannelReceiver({});
...
it("rejects non-connect requests", (t) => {
	const channelReceiver = new StandaloneChannelReceiver({});
	// @ts-expect-error - taking a shortcut by accessing protected property
	const postResponseStub = sinon.stub(channelReceiver, "postResponse");
...
it("forwards response messages to default message handler when not ready", (t) => {
	const channelReceiver = new StandaloneChannelReceiver({});
	// @ts-expect-error - taking a shortcut by accessing protected property
...
it("doesn't forward response messages to default message handler once ready", (t) => {
	const channelReceiver = new StandaloneChannelReceiver({});
	// @ts-expect-error - taking a shortcut by accessing protected property
	const onMessageStub = sinon.stub(channelReceiver, "onMessage");
